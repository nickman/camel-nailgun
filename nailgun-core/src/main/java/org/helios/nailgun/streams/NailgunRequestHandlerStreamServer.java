...
import org.helios.nailgun.handler.NailgunStreamHandler;
...
/**
 * <p>Title: NailgunRequestHandlerStreamServer</p>
 ...
 * <p><code>org.helios.nailgun.streams.NailgunRequestHandlerStreamServer</code></p>
 */

public class NailgunRequestHandlerStreamServer {
	/** The singleton instance */
	private static volatile NailgunRequestHandlerStreamServer instance = null;
	...
	/**
	 * Returns the singleton NailgunRequestHandlerStreamServer instance
	 * @return the singleton NailgunRequestHandlerStreamServer instance
	 */
	public static NailgunRequestHandlerStreamServer getInstance() {
		if(instance==null) {
			synchronized(lock) {
				if(instance==null) {
					instance = new NailgunRequestHandlerStreamServer();
				}
			}
		}
		return instance;
	...
	/**
	 * Creates a new NailgunRequestHandlerStreamServer
	 */
	private NailgunRequestHandlerStreamServer() {
	...
	/**
	 * Creates a new StreamServer through which the NailgunRequestDecoder can locate the correct handler for a given command,
	 ...
	 */
	/*
	 * NOTE: Right now, NailgunRequestHandlers MUST implement NailgunStreamHandler
	 * but we want to separate the two, so that if NailgunStreamHandler is NOT implemented,
	...
	public void newCommandHandlerStreamServer(NailgunRequestHandler requestHandler) {
		Map<String, ChannelHandler> channelHandlers = ((NailgunStreamHandler<?>)requestHandler).getChannelHandlers();
		...
		if(sb==null) {
			synchronized(commandServers) {
				...
				ServerBootstrap sboot = new ServerBootstrap(new NailgunRequestChannelFactory(Channels.pipelineFactory(pipeline)));  
