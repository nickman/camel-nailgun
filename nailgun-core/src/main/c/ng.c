...
void initArray(Array *a) {
  a->used = 0;
  a->size = 0;
  a->str = malloc(sizeof(char **));
}

void insertArray(Array *a, char *element) {
  if (a->used == a->size) {
    a->size *= 2;
    a->str = (char **)malloc(strlen(element) * sizeof(char));
  }
  strcpy(a->str[a->used++], element);
}
...
/**
 * Sends the contents of the specified file as a long argument (--file)
...
void usage(int exitcode) {
  ...
  fprintf(stderr, "Usage: ng class [--options] [args]\n");
  fprintf(stderr, "          (to execute a class)\n");
  fprintf(stderr, "   or: ng alias [--options] [args]\n");
  fprintf(stderr, "          (to execute an aliased class)\n");
  fprintf(stderr, "   or: alias [--options] [args]\n");
  ...
  fprintf(stderr, "   --port                    to specify the port of the nailgun server\n");
  fprintf(stderr, "                                 (default is NAILGUN_PORT environment variable\n");
  fprintf(stderr, "                                 if set, otherwise 2113)\n");
  fprintf(stderr, "   --file FILE               places the entire contents of FILE into the\n");
...
int main(int argc, char *argv[], char *env[]) {
  ...
  debug("Nailgun Client Debug Version");
  ...
  /* quite possibly the lamest commandline parsing ever.
     look for the two args we care about (--server and
     --port) and NULL them and their parameters after
